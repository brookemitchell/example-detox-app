...
describe(":ios: User Activity", () => {
  it("Init from browsing web", async () => {
    ...
    await device.launchApp({
      newInstance: true,
      userActivity: userActivityBrowsingWeb
    });
    await expect(element(by.text("https://my.deeplink.dtx"))).toBeVisible();
  });

  it.skip("Background searchable item", async () => {
    await device.launchApp({ newInstance: true });
    await device.sendToHome();
    await device.launchApp({
      newInstance: false,
      userActivity: userActivitySearchableItem
    });
...
userActivitySearchableItem.userInfo[
  DetoxConstants.searchableItemActivityIdentifier
] = "com.test.itemId";
