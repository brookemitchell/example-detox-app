describe("Matchers", () => {
	...
	it.skip(":ios: should match elements by accesibility trait", async () => {
	...
	it("should match elements by using two matchers together with and", async () => {
		...
		await expect(
			element(by.id("UniqueId345").and(by.text("RandomJunk")))
		).toNotExist();
		await expect(
		...
		if (device.getPlatform() === "ios") {
			await expect(
				element(by.id("UniqueId345").and(by.traits(["button"])))
			).toNotExist();
		}
	});
	...
	it.skip("should choose from multiple elements matching the same matcher using index", async () => {
		await expect(element(by.text("Product")).atIndex(2)).toHaveId(
			"ProductId002"
		);
	});
});
